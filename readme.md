# THIS repo serves as a base docker proj for the Dev & Prod of Laravel/Nginx/Mysql/Redis

# the repo is based on https://github.com/systemsdk/docker-nginx-php-laravel.git, but with different structure
...
## Structure
.
|-- .env                        => the enviroment variable setting file, which is used by Makefile, excluded from git for sensitive data, copy .env.example to .env for quick start
|-- .env.example                => an example of .env file
|-- Makefile                    => the makefile, type 'make' for details
|-- docker                      => the docker related dockerfile and container service config files
|-- docker-compose-prod.yml     => docker-compose prj file for production
|-- docker-compose-staging.yml  => docker-compose prj file for staging
|-- docker-compose-test-ci.yml  => docker-compose prj file for test-ci
|-- docker-compose.yml          => docker-compose prj file for development
|-- docs                        => documentations
|-- laravel                     => the Laravel source root, Caution: the ".env" and ".env.xxx" file in this dir have args passed through by file ".env" from root dir
|-- readme.md                   => readme
`-- storage                     => mount point for persistent storage of data in container

## Build
### firstly make sure .env file exist in the root dir
### secondly change any settings in .env, especially the "ENV" var
```bash
make build
make start
